...
# Replace zeros in hurricane_data_cleaned
hurricane_data_cleaned['Pressure'] = hurricane_data_cleaned['Pressure'].replace(0, 1013).fillna(1013)
hurricane_data_cleaned['Dist2Land'] = hurricane_data_cleaned['Dist2Land'].replace(0, np.nan).fillna(
    hurricane_data_cleaned['Dist2Land'].mean()
)
hurricane_data_cleaned['Wind_Speed'] = hurricane_data_cleaned['Wind_Speed'].replace(0, np.nan).fillna(
    hurricane_data_cleaned['Wind_Speed'].mean()
)
...
# Replace zeros and NaNs in derived columns
final_merged_data['Avg_Pressure'] = final_merged_data['Avg_Pressure'].replace(0, 1013).fillna(1013)
final_merged_data['Avg_Dist2Land'] = final_merged_data['Avg_Dist2Land'].replace(0, np.nan).fillna(
    final_merged_data['Avg_Dist2Land'].mean()
)
final_merged_data['Avg_Wind_Speed'] = final_merged_data['Avg_Wind_Speed'].replace(0, np.nan).fillna(
    final_merged_data['Avg_Wind_Speed'].mean()
)
